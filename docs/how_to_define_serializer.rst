...
.. warning::
    ...
    シリアライザーの定義中にvalidate_methodを記述できることは、便利な半面、常に危険を伴います。
    例えば、シリアライザーの定義を一般のユーザーに記述させたい要望があったとしましょう。
    もし、一般ユーザーがコード中にシンタックスエラーを含む内容を記述してしまうと、500エラーでクラッシュしてしまいます。

    また、悪意のあるコードを書かれてしまうと、素直にそれが実行されてしまいます！

    もし、一般ユーザーにシリアライザーを記述させたい場合は、 ``allow_validate_method`` オプションを ``False`` にして
    ``validate_method`` の記述を不許可にすることを強くおすすめします。
